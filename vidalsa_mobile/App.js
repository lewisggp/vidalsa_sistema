...
function PantallaLogin({ onLogin }) {
  ...
  const guardarIp = async () => {
    ...
    if (!ipLimpia) {
      Alert.alert("Error", "Escribe una IP o dirección válida.");
      return;
    }
...
function PantallaMovilizaciones({ user, onOpenMenu }) {
  ...
  const registrarMovimiento = async () => {
    if (equiposSel.length === 0) {
      Alert.alert("Atención", "Selecciona al menos un equipo.");
      return;
    }
    if (!frenteDest) {
      Alert.alert("Atención", "Selecciona el frente de destino.");
      return;
    }
    setGuardando(true);
    try {
