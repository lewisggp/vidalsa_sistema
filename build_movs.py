...
filepath = 'vidalsa_mobile/App.js'
with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

new_component = """// ─── PANTALLA DE MOVILIZACIONES ───────────────────────────────────────────────
          ...
          <View>
             ...
             {cargandoHist && historial.length === 0 ? (
               ...
               <View style={{ alignItems: 'center', marginTop: 40, opacity: 0.5 }}>
                   ...
                   <Text style={{ color: '#64748b', marginTop: 10 }}>No hay movilizaciones registradas.</Text>
               </View>
             ) : (
               <View>
                 ...
                 {historialesFiltrados.map((h, i) => (
                    <View key={h.ID_MOVILIZACION || i} style={{ backgroundColor: '#fff', borderRadius: 12, padding: 15, marginBottom: 15, borderWidth: 1, borderColor: '#e2e8f0', shadowColor: '#000', shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 }}>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                        <View style={{ width: 45, height: 45, borderRadius: 8, backgroundColor: '#f1f5f9', justifyContent: 'center', alignItems: 'center', marginRight: 12, borderWidth: 1, borderColor: '#e2e8f0' }}>
...
start_str = "// ─── PANTALLA DE MOVILIZACIONES ───────────────────────────────────────────────"
end_str = "// ─── APP PRINCIPAL ────────────────────────────────────────────────────────────"

start_idx = content.find(start_str)
end_idx = content.find(end_str)

if start_idx != -1 and end_idx != -1:
    updated = content[:start_idx] + new_component + content[end_idx:]
